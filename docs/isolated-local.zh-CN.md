...
- 固定 Pi `@earendil-works/pi-coding-agent@0.85.1`、Node.js `22.19.0`，
  Git `2.39.5`（Debian `1:2.39.5-0+deb12u3`）、Pi 原生 `deepseek` Provider 和 `deepseek-v4-pro` 模型。用户自备有模型访问权限的 DeepSeek API key。
...
镜像提供固定 Playwright `1.62.0` 及其对应版本的 Chromium headless shell，
用于检查同一容器内运行的应用。准备环境时按提交的依赖锁安装浏览器工具，
并在执行资源限制下实际打开回环地址测试页面，验证键盘操作、刷新持久化、
窄屏视口和截图。探测失败则准备不完成。没有浏览器资格记录的旧环境需要重新准备；
已有任务仍绑定原来的镜像身份。

Agent 可在允许修改的范围内编写 `.mjs` 测试，使用镜像提供的模块：
...
try {
  ...
  await page.goto('http://127.0.0.1:3000'); // 先在同一容器内启动应用。
  // 在这里断言任务要求的实际操作和结果。
} finally {
  await browser.close();
}
...
不需要在任务仓库安装浏览器依赖。auto 检查策略下，执行指引要求对页面行为变更运行
相关浏览器检查；named 和 none 策略仍优先。每条检查须从明确的仓库工作目录单独执行，
并列入所选检查。测试后停止应用服务器，截图和临时输出放在仓库副本之外的 `/tmp`；
仅导入范围内的普通文本变更。

Chromium 在 Chora 现有 Docker 边界内运行，不启用浏览器自身的嵌套沙箱。
这不会增加宿主浏览器访问、共享宿主 IPC、端口发布或 Docker 权限。
此工具用于测试任务自己的应用；该资格探测不代表支持任意网页浏览。

结果页将验收覆盖与命令结果分开呈现。所选检查通过不能证明所有要求均已测试，
**待评审**也不是验收通过。请逐项核对原始要求和实际证据，包括页面行为；缺失证据的
部分仍未验证。浏览器检查仍是 Agent 自报；环境探测验证的是工具可用性，不是 Agent 的产出。

## 边界和恢复

写入有限日志前会过滤 Pi 模型和工具的临时增量进度，保留最终消息和工具结果。
真实证据超限时以 `runtime_output_limit_exceeded` 失败，不导入部分修改。
执行中发现容器停止、暂停或无法观察时进入恢复状态，避免一直显示运行中；
观察失败不等于已证明进程死亡，也不授权启动替代执行。
...
模型请求超时时，分别检查宿主机、Docker VM 和 bridge 网络容器的出站 HTTPS。
宿主机可连接不代表容器可连接。先恢复 VM 的网络路由再 Resume；更换模型无法修复网络超时。
如果改用另一台 Docker 引擎，应在独立数据目录重新准备并创建新 Task，保留已有 Task
冻结的引擎与镜像身份。
