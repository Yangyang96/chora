...
状态：**源码已公开，M2-S3 技术验收已完成**。[路线图](../ROADMAP.zh-CN.md)记录
冻结验收基线。后续每次发布仍须验证准确候选及受影响改动，源码公开不代表任意
后续提交自动通过验收。M1 私有隔离输入和未验收的安装生命周期不构成
Local Connected 的公开启动前置。
...
- [x] M2-S2-P1..P4 的 HTTP/基线/文件/检查基础验收。
- [x] M2-S3 文档化启动、连续任务、数据维护及公共 CI 验收。
- [x] 包含的 Commit/Push/PR/Merge/清理及旧 Apply 已验收。
- [x] 精确导出的验收候选不依赖 M1 私有输入，复现文档化本机 Pi 路线。

这些已完成门禁绑定保留的验收候选，不代表每个后续 HEAD。上方未勾选的发布检查
需要针对每个新候选重新评估。
