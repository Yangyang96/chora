...
当前开发源码使用 SQLite schema 49，rc.10 使用 schema 41。
schema 46–49 增加持久化的本地委派授权、父子关联、不可变 Agent 计划来源、单次规划授权及冻结综合来源，
不改写已有 Task、Run 或结果历史。
旧二进制会拒绝这个较新的 schema；回退时须使用升级前的完整数据根目录备份。
41 → 42 在修改数据库前自动创建一致性快照 `.chora-schema41-backup-*`，加入本地隔离契约。
升级前请停止所有使用者并备份整个数据根目录；回退须使用匹配的代码，在原绝对路径恢复升级前数据。
...
这个数据库快照不能代替停服后的整个数据根目录备份。回退时恢复升级前完整数据并使用
匹配的 rc.10 源码，不得编辑迁移账本或让旧程序写入新数据库。
已有隔离任务还需保留确切的准备镜像；清理镜像后，必须恢复同一镜像才可继续。
开发验证只迁移测试 fixture，没有迁移现有用户服务或数据。
