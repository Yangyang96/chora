...
export type ModelBinding = { catalog: { agentId: string; runtimeIdentity: string; runtimeVersion: string; models: Array<{ provider: string; modelId: string }>; digest: string }; provider: string; modelId: string; selectedAt: string; source: 'coding_agent' }
export type ModelIdentity = { provider: string; modelId: string }
export type ProjectExecutionSettings = { version: number; agentExecutionProfile: 'trusted_local' | 'isolated_local'; model: ModelIdentity | null }
export type TaskExecutionSettingsInput = { projectVersion: number; agentExecutionProfile?: 'trusted_local' | 'isolated_local'; model?: ModelIdentity | null }
export type TaskExecutionSettingsSnapshot = { projectId: string; projectVersion: number; agentExecutionProfile: 'trusted_local' | 'isolated_local'; environmentSource: 'project' | 'default' | 'task'; modelSource: 'project' | 'default' | 'task'; modelBinding: ModelBinding | null }
export type TaskRef = { id: string; roomId?: string; title?: string; goal?: string; executionProfile?: TaskExecutionProfile; agentExecutionProfile?: AgentExecutionProfile; modelBinding?: ModelBinding; executionSettings?: TaskExecutionSettingsSnapshot; repository?: RepositoryIdentity; worktree?: TaskWorktree; criteria?: Array<{ id: string; title: string; description?: string }>; selection?: SelectionView; planning?: TechnicalPlanning; frozenSnapshot?: FrozenSnapshotRef }
