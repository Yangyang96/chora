...
test('freezes a planning requirement only after selecting research delegation', async () => {
 ...
 await userEvent.click(screen.getByLabelText('Generate one synthesis after research'))
 ...
 expect(onSubmit.mock.calls[0][6]).toMatchObject({ synthesize: true, outcomeKind: 'document', materials: [{ locator: 'supplied:notes' }] })
 await userEvent.click(screen.getByLabelText('Research / write a proposal'))
 await userEvent.click(screen.getByRole('button', { name: 'Start' }))
 expect(onSubmit.mock.lastCall?.[0]).toBe('Compare compatibility options')
 expect(onSubmit.mock.lastCall?.[6]).not.toHaveProperty('synthesize')
})
