...
describe('NewApp', () => {
 ...
 test('opening an unstarted Task after reload is read-only until explicit Start Run', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (path.endsWith('/tasks/task-1')) return jsonResponse({ id: 'task-1', roomId: 'room-1', title: 'Saved task', agentExecutionProfile: 'trusted_local', executionSettings: { projectId: 'project-at-creation', projectVersion: 3, agentExecutionProfile: 'trusted_local', environmentSource: 'project', modelSource: 'task', modelBinding: { catalog: { agentId: 'pi', runtimeIdentity: 'pi', runtimeVersion: '1', models: [], digest: 'd' }, provider: 'openai', modelId: 'frozen-model', selectedAt: '2026-09-20T00:00:00Z', source: 'coding_agent' } }, planning: { revisions: [], draft: { id: 'draft-1', editVersion: 1, content: { technical_steps: ['Saved step'], decisions: [], risks: [], unknowns: [] } } } })
    ...
    expect(screen.getAllByText(/Execution environment/).some((element) => element.textContent === 'Execution environment · Local execution · No Sandbox')).toBe(true)
    ...
    expect(screen.getByText(/openai · frozen-model/)).toHaveTextContent('task override')
    await userEvent.click(screen.getByRole('button', { name: '中文' }))
    expect(screen.getAllByText('本机执行 · 无沙箱').some((element) => element.closest('p')?.textContent === '执行环境 · 本机执行 · 无沙箱')).toBe(true)
...
test('starts a v2 resource Task without a scalar worktree and makes preparation cancellable', async () => {
  ...
  const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (method === 'GET' && path === '/api/projects/project-1/execution-settings') return jsonResponse({ version: 0, agentExecutionProfile: 'isolated_local', model: null })
    if (method === 'GET' && path === '/api/rooms') return jsonResponse({ activeRooms: [roomSummary], archivedRooms: [] })
...
test('can cancel resource preparation before Task creation returns an ID', async () => {
  ...
  await userEvent.click(await screen.findByLabelText('Override Project defaults for this task'))
  await userEvent.click(screen.getByRole('radio', { name: LOCAL_EXECUTION_LABEL }))
  await userEvent.click(screen.getByRole('button', { name: 'Start' }))
