...
已接受的下一步先简化执行设置，再加入软件项目内的调研、方案和文档协作。
仓库可选的 Task 尚在规划，当前不可用。见[项目工作流程](docs/project-workflows.zh-CN.md)
和[下一阶段排期](ROADMAP.zh-CN.md#下一阶段个人工作空间切片)。
...
截至 **2026 年 9 月 20 日**，源码已公开，仍处于开发者 Alpha。
...
| 已实现 | 公开 Isolated Local 准备、任务看板 V0，以及 Local Connected 的项目 Skills/MCP。 |
| 尚待完成 | 最终产品用户验收和正式 macOS 分发；已发布预发布版本不等于稳定版。 |
| 接下来 | 简化执行设置，再支持软件项目内的调研、方案和文档协作。 |
| 更后续 | 团队和多 Agent 协作；按需加入后台/远程执行及其他 Provider。 |
...
[Isolated Local](docs/isolated-local.zh-CN.md) 已可在受支持主机上通过公开输入准备，
项目、依赖和凭证限制见指南；该路径不需要历史 M1 私有环境。
其他服务商和部署方式仍属后续工作，见[路线图](ROADMAP.zh-CN.md)。
