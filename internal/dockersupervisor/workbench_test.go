...
func workbenchArguments() []string {
	return []string{"--mode", "rpc", "--no-session", "--no-extensions", "--no-skills", "--no-prompt-templates", "--no-themes", "--no-context-files", "--provider", "deepseek", "--model", "deepseek-v4-pro", "--extension", "/opt/chora/resource_check_observer.mjs"}
}
