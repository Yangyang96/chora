...
func TestV28UpgradePreservesPopulatedLocalReviewResult(t *testing.T) {
	...
	ddl := "BEGIN; DROP TABLE delegation_planning_intents; DROP TABLE delegation_proposal_sources; DROP TABLE task_delegation_children; DROP TABLE task_delegations; DROP TRIGGER local_review_decisions_authority;\n" + create +
	...
	if version, err := upgraded.SchemaVersion(ctx); err != nil || version != 48 {
