...
func TestOpenMigratesToCurrentVersionAndReopens(t *testing.T) {
	...
	if got, err := db.SchemaVersion(context.Background()); err != nil || got != 43 {
...
func TestMigrationLedgerFailsClosedOnDriftGapAndNewerSchema(t *testing.T) {
	for _, tc := range []struct{ name, mutate string }{
		...
		{"newer", `INSERT INTO schema_migrations(version,name,checksum,applied_at) VALUES(44,'0044_future.sql',zeroblob(32),'2026-01-01T00:00:00Z')`},
	} {
