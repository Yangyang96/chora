...
func (r *reader) GetTaskBoardSnapshot(ctx context.Context, id domain.ProjectID) (storecontract.TaskBoardSnapshot, error) {
 ...
 coalesce(json_extract((CASE WHEN json_valid(s.canonical_json) THEN s.canonical_json ELSE '{}' END),'$.outcomeKind'),''),
 coalesce((SELECT coalesce(review.kind,'pending') FROM project_document_revisions revision LEFT JOIN project_document_reviews review ON review.revision_id=revision.id WHERE revision.task_id=task.id ORDER BY revision.revision_number DESC LIMIT 1),''),
 ...
 FROM tasks task JOIN rooms room ON room.id=task.room_id LEFT JOIN task_resource_snapshots s ON s.task_id=task.id LEFT JOIN current_runs r ON r.task_id=task.id LEFT JOIN attempts a ON a.run_id=r.id AND a.sequence=r.current_attempt_number LEFT JOIN resource_result_groups g ON g.attempt_id=a.id WHERE room.project_id=?`, id.String(), id.String())
	...
	for rows.Next() {
		...
		var outcomeKind, documentStatus string
		if err = rows.Scan(&task, &snapshot, &outcomeKind, &documentStatus, &invalid); err != nil {
			break
		}
		if f := index[task]; f != nil {
			f.ResourceSnapshot = snapshot
			f.OutcomeKind, f.DocumentStatus = outcomeKind, documentStatus
			if invalid {
