...
func TestModelProvenanceObservesNativeAssistantMessageWithoutStartupIdentity(t *testing.T) {
	...
	events := []domain.RunEvent{
		...
		provenanceEvent(t, runID, 5, "assistant_message", "adapter", `{"model_id":"deepseek-v4-pro","provider":"deepseek"}`),
		provenanceEvent(t, runID, 6, "assistant_message", "adapter", `{"model_id":"deepseek-v4-pro","provider":"deepseek"}`),
	...
	if observed.Status != "observed" || len(observed.Identities) != 1 || observed.Identities[0] != (modelIdentityView{Provider: "deepseek", ModelID: "deepseek-v4-pro"}) {
