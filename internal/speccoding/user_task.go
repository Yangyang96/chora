...
func deterministicInitialPlanContent(intent frozenUserTask) domain.TechnicalPlanContent {
	if intent.Resources != nil {
		...
		return domain.TechnicalPlanContent{TechnicalSteps: []string{"Inspect the selected repositories and relevant modules.", "Implement the requirement within the frozen repository roles and optional limits.", "Select and report checks under each frozen policy, then produce grouped reviewable changes."}, Decisions: []string{"Use the selected Pi execution environment; original checkouts stay unchanged until explicit delivery."}, Risks: []string{"Checks may be unavailable or fail; report evidence and unknowns explicitly."}, Unknowns: []string{"Relevant files and applicable checks will be identified during the task."}}
	}
	return domain.TechnicalPlanContent{
