...
func ValidateAttemptSuccessor(current, next Attempt) error {
	...
	if current.ID() != next.ID() || current.RunID() != next.RunID() || current.Sequence() != next.Sequence() || currentPredecessor != nextPredecessor || currentHasPredecessor != nextHasPredecessor || current.ContextSnapshotID() != next.ContextSnapshotID() || current.ContextDigest() != next.ContextDigest() || current.AdapterID() != next.AdapterID() || current.AgentExecutionProfileBinding() != next.AgentExecutionProfileBinding() || current.ModelBinding() != next.ModelBinding() || current.ExternalSession() != next.ExternalSession() || current.RetryReason() != next.RetryReason() || current.InterventionReason() != next.InterventionReason() || current.ContextDelta() != next.ContextDelta() || !current.CreatedAt().Equal(next.CreatedAt()) {
