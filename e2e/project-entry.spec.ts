...
test('Project execution defaults persist across restart and require explicit Task host acknowledgement', async ({ page, request }) => {
  const repo = await createGitRepo(`execution-settings-${runSuffix}`)
  const project = await addProject(request, repo)
  const settingsURL = `/api/projects/${project.id}/execution-settings`
  await page.goto(`/projects/${project.id}`)
  await page.getByText('Default execution settings', { exact: true }).click()
  const panel = page.locator('details').filter({ has: page.getByText('Default execution settings', { exact: true }) })
  await expect(panel.getByRole('radio', { name: 'Isolated execution', exact: true })).toBeChecked()
  await panel.getByRole('radio', { name: 'Local execution · No Sandbox', exact: true }).click()
  await expect(page.getByRole('dialog')).toHaveCount(0)
  await panel.getByRole('button', { name: 'Save execution defaults', exact: true }).click()
  await expect.poll(async () => (await getJSON<{version:number}>(request, settingsURL)).version).toBe(1)
  const saved = await getJSON<{version:number;agentExecutionProfile:string;model:unknown}>(request, settingsURL)
  expect(saved.agentExecutionProfile).toBe('trusted_local')
  expect(saved.model).toBeNull()
  const ack = await getJSON<{acknowledged:boolean}>(request, '/api/agent-execution/trusted-local-acknowledgements/current')
  expect(ack.acknowledged).toBe(false)

  const other = await addProject(request, await createGitRepo(`execution-other-${runSuffix}`))
  const untouched = await getJSON<{version:number;agentExecutionProfile:string}>(request, `/api/projects/${other.id}/execution-settings`)
  expect(untouched.version).toBe(0)
  expect(untouched.agentExecutionProfile).toBe('isolated_local')
  await stopServer('SIGTERM')
  await startServer()
  expect(await getJSON(request, settingsURL)).toEqual(saved)
  ...
  await page.getByRole('button', { name: 'Review and acknowledge Local execution', exact: true }).click()
  ...
  await expect(page.getByRole('button', { name: 'Start', exact: true })).toBeDisabled()
  expect((await getJSON<{acknowledged:boolean}>(request, '/api/agent-execution/trusted-local-acknowledgements/current')).acknowledged).toBe(false)
  await page.getByRole('checkbox', { name: 'Override Project defaults for this task' }).check()
  await page.getByRole('radio', { name: 'Isolated execution', exact: true }).check()
  expect((await getJSON<{agentExecutionProfile:string}>(request, settingsURL)).agentExecutionProfile).toBe('trusted_local')
  await page.getByRole('checkbox', { name: 'Override Project defaults for this task' }).uncheck()
  await expect(page.getByRole('radio', { name: 'Local execution · No Sandbox', exact: true })).toBeChecked()
})
